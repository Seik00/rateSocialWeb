...
def getTick(platform, api_info, market):
    ...
    cur.execute("SELECT * from jl_spot_market where platform = '%s' and market_name ='%s'" % (
        platform, market))
    robot = cur.fetchone()
    return robot['price']
    ...
    try:
        ...
        if platform == 'okex':
            ...
            if result['fee']['currency'] =='BNB':
                print(result['fee']['currency'])
                bnb_price = float(getTick('binance', '', 'BNB/USDT'))
                cost = result['fee']['cost']*bnb_price
                print('cost',cost)
                print('price',result['price'])
                order_info['deal_fee'] = cost/result['price']
            else:
...
def updateMsg(robot_id, show_msg, show_msg_en, show_msg_vn):
    conn = Conn()
    cur = conn.cursor()
    update_sql = "update jl_quant_robot set show_msg = '%s' , show_msg_en = '%s', show_msg_vn = '%s' where id = %d" % (
        show_msg, show_msg_en, show_msg_vn, robot_id)
    print(update_sql)
...
def restockFinish(robot_id):
    conn = Conn()
    cur = conn.cursor()
    update_sql = "update jl_quant_robot set is_restock = 0 where id = %d" % robot_id
    print(update_sql)
    cur.execute(update_sql)
    conn.commit()
    cur.close()
...
def insertLog(platform, robot_id, uid, log,content_log,content_log_vn):
    conn = Conn()
    cur = conn.cursor(MySQLdb.cursors.DictCursor)
    cur.execute(
    ...
    if last_log:
        if last_log['content'] == log:
            return
    insert_sql = "insert jl_quant_robot_log(platform,uid,qrobot_id,content,content_log,content_log_vn) values ('%s',%d,%d,'%s','%s','%s')" % (
        platform, uid, robot_id, log, content_log,content_log_vn)
    print(insert_sql)
...
def onTick(robot_id, market_info, api_info):
    ...
    is_restock = int(robot_config['is_restock'])
    # print(market)
    #check thread
    print(thread_alive)
    ...
    print('-----market_price-----',tick_price)
    if tick_price is None:
        thread_alive[robot_id] = 0
        return
    first_order_double = float(robot_config['first_order_double'])    
    if first_order_double == 1:
        first_order_value = float(robot_config['first_order_value'])*2
    else:
    ...
    if values_str:
        ...
        revenue = up_num * 100/base_price
        print('deal_amount : ',values_dict['deal_amount'])
        print('deal_money : ',values_dict['deal_money'])
        ...
        if is_clean == 1:
            ...
            if rst['code'] == 1:
                ...
                updateMsg(robot_id, u'清仓卖出成功', u'Successful clearance', u'Successful clearance')
                ...
                if recycle_status == 0:
                    ...
                    insertLog(platform, robot_id,
                              uid, u"量化机器人已经关闭", u"Robot shut down", u"Robot shut down")
                else:
                    insertLog(platform, robot_id,
                              uid, u"量化机器人已经重新开启", u"Robot restart", u"Robot restart")
                thread_alive[robot_id] = 0
                return
            else:
                if(platform == 'binance' and '-1021' in rst['msg']):
                    print('----------------1021 ================')  
                else:
                    disableRobot(robot_id)

                if(platform == 'binance' and '-2015' in rst['msg']):
                    updateMsg(robot_id, u'API-key 错误，ip或 权限不足', u'Invalid API-key, IP, or permissions for action.', u'Invalid API-key, IP, or permissions for action.')
                    insertLog(platform, robot_id, uid, u'API-key 错误，ip或 权限不足', u'Invalid API-key, IP, or permissions for action.', u'Invalid API-key, IP, or permissions for action.')
                  
                elif(platform == 'binance' and 'binance Account has insufficient balance for requested action.' in rst['msg']):
                    updateMsg(robot_id, u'币安余额不足', u'binance Account has insufficient balance for requested action.', u'binance Account has insufficient balance for requested action.')
                    insertLog(platform, robot_id, uid, u'币安余额不足', u'binance Account has insufficient balance for requested action.', u'binance Account has insufficient balance for requested action.')
                    
                elif(platform == 'binance' and 'MIN_NOTIONAL' in rst['msg']):
                    updateMsg(robot_id, u'交易失败,请查询币安交易规则', u'Trade fail ,please check binance trade roles', u'Trade fail ,please check binance trade roles')
                    insertLog(platform, robot_id, uid, u'交易失败,请查询币安交易规则', u'Trade fail ,please check binance trade roles', u'Trade fail ,please check binance trade roles')
                
                elif(platform == 'huobi' and '[API Key没有权限]' in rst['msg']):
                    updateMsg(robot_id, u'API-key 错误，ip或 权限不足', u'Invalid API-key, IP, or permissions for action.', u'Invalid API-key, IP, or permissions for action.')
                    insertLog(platform, robot_id, uid, u'API-key 错误，ip或 权限不足', u'Invalid API-key, IP, or permissions for action.', u'Invalid API-key, IP, or permissions for action.')
                   
                elif(platform == 'huobi' and 'trade account balance is not enough' in rst['msg']):
                    updateMsg(robot_id, u'火币余额不足', u'Huobi Account has insufficient balance for requested action.', u'Huobi Account has insufficient balance for requested action.')
                    insertLog(platform, robot_id, uid, u'火币余额不足', u'Huobi Account has insufficient balance for requested action.', u'Huobi Account has insufficient balance for requested action.')
                
                else:
                    updateMsg(robot_id, u'清仓卖出失败' + rst['msg'], u"Clearance fail " + rst['msg'], u"Clearance fail " + rst['msg'])
                    insertLog(platform, robot_id, uid,
                            u'清仓卖出失败' + rst['msg'], u"Clearance fail " + rst['msg'], u"Clearance fail " + rst['msg'])
        ...
        if is_restock == 1:
            ...
            #    custom_cover_times = float(custom_cover[values_dict['order_count']-1]['times'])
            #    put_order = float(values_dict['deal_money'])*custom_cover_times
            #else:
            if first_order_double == 1:
                put_order = float(values_dict['deal_money'])*2
            else:
                put_order =  values_dict['deal_money']
           

            if platform == 'sinance':
                token = api_info['secret_key']
                rst = localOrder(2, token, market_info['market'], put_order)
            else:
                rst = order(robot_id,2, api_info, market, put_order)

            restockFinish(robot_id)
            if rst['code'] == 1:
                print('retockrst:',rst)
                ...
                values_dict['deal_amount'] = float(values_dict['deal_amount']) + float(deal_amount)
                values_dict['deal_money'] = float(values_dict['deal_money']) + float(deal_money)
                values_dict['base_price'] = float(values_dict['deal_money'] / values_dict['deal_amount'])
                values_dict['last_price'] = price
                ...
                updateMsg(robot_id, u'手动补仓成功', u'Successfully Cover', u'Successfully Cover')
                insertLog(platform, robot_id, uid, u"手动补仓成功：单数 %s , 成交总价 %s %s ,成交均价 %s %s,成交数量 %s %s" % (
                                    values_dict['order_count'], deal_money, money_name, price, money_name, deal_amount, stock_name), u"Successful manual replenishment: order %s , Total price %s %s ,Average price %s %s,Amount %s %s" % (
                                    values_dict['order_count'], deal_money, money_name, price, money_name, deal_amount, stock_name), u"Successful manual replenishment: order %s , Total price %s %s ,Average price %s %s,Amount %s %s" % (
                ...
                if(platform == 'binance' and '-2015' in rst['msg']):
                    ...
                    updateMsg(robot_id, u'补仓失败 :币安余额不足', u'Cover fail : binance Account has insufficient balance for requested action.', u'Cover fail : binance Account has insufficient balance for requested action.')
                    insertLog(platform, robot_id, uid, u'补仓失败 :币安余额不足', u'Cover fail : binance Account has insufficient balance for requested action.', u'Cover fail : binance Account has insufficient balance for requested action.')
                    ...
                    updateMsg(robot_id, u'补仓失败 :火币余额不足', u'cover fail :Huobi Account has insufficient balance for requested action.', u'cover fail :Huobi Account has insufficient balance for requested action.')
                    insertLog(platform, robot_id, uid, u'补仓失败 :火币余额不足', u'cover fail :Huobi Account has insufficient balance for requested action.', u'cover fail :Huobi Account has insufficient balance for requested action.')
                
                else:
                    updateMsg(robot_id, u'补仓失败 : ' + rst['msg'], u'cover fail : ' + rst['msg'], u'cover fail : ' + rst['msg'])
                    insertLog(platform, robot_id, uid,
                                            u'补仓失败 : ' + rst['msg'],
                    ...
                    if tick_price > up_price:
                        ...
                        if down_rate >= stop_profit_callback_rate:
                            ...
                            if rst['code'] == 1:
                                ...
                                updateValues(robot_id, '')
                                updateRevenue(robot_id, 0)
                                updateMsg(robot_id, u'卖出成功', u'Successfully sold', u'Successfully sold')
                                ...
                                if recycle_status == 0:
                                    ...
                                    disableRobot(robot_id)
                                    insertLog(platform, robot_id,
                                            uid, u"量化机器人已经关闭", u"Quantitative robot has been closed", u"Quantitative robot has been closed")
                                else:
                                    insertLog(platform, robot_id,
                                            uid, u"量化机器人已经重新开启", u"Quantitative robot has restart", u"Quantitative robot has restart")
                                ...
                                if(platform == 'binance' and '-2015' in rst['msg']):
                                    ...
                                    updateMsg(robot_id, u'卖出失败' + rst['msg'], u'Sell fail' + rst['msg'], u'Sell fail' + rst['msg'])
                                    insertLog(platform, robot_id, uid,
                                            u'卖出失败' + rst['msg'],
                                            u'Sell Fail' + rst['msg'],
                                            u'Sell Fail' + rst['msg'])
            ...
            if tick_price < last_price:
                ...
                if trend_side == 0 or trend_side == 1:
                    ...
                    if custom_cover:
                        cover_rate = float(custom_cover[values_dict['order_count']-1]['percentage'])
                    ...
                    if tick_price < down_price:
                        ...
                        if up_rate >= cover_callback_rate:
                            ...
                            if order_count == 1:
                                ...
                                if custom_cover:
                                    put_order =float(values_dict['deal_money'])*float(custom_cover[values_dict['order_count']-1]['times'])
                                else:
                                    if first_order_double == 1:
                                        put_order = float(values_dict['deal_money'])*2
                                    else:
                            ...
                            if rst['code'] == 1:
                                ...
                                if values_dict['order_count'] >= max_order_count:
                                    ...
                                    insertLog(platform, robot_id, uid, u"达到最大做单数量次数：单数 %s " %
                                            ...
                                            values_dict['order_count'])
                                updateValues(robot_id, json.dumps(values_dict))
                                updateMsg(robot_id, u'补仓成功', u'Successfully Cover', u'Successfully Cover')
        ...
        if rst['code'] == 1:
            ...
            values_dict = {'first_order_price': price, 'base_price': price, 'up_price': 0, 'down_price': 0, 'trend_side': 0,
                           'order_count': 1, 'deal_amount': deal_amount, 'deal_money': deal_money, 'order_finish': 0, 'pid': pid, 'last_price': price}
                           
            updateValues(robot_id, json.dumps(values_dict))
            updateMsg(robot_id, u'下单成功', u'Order Success', u'Order Success')
            ...
            if(platform == 'binance' and '-2015' in rst['msg']):
                ...
                updateMsg(robot_id, u'首单开单失败 : ' + rst['msg'], u'First order fail : ' + rst['msg'], u'First order fail : ' + rst['msg'])
                insertLog(platform, robot_id, uid, u'首单开单失败 : ' + rst['msg'], u'First order fail : ' + rst['msg'], u'First order fail : ' + rst['msg'])
...
def loadStrategyConfig(cur):
    ...
    # print(api_list)
    cur.execute('SELECT * from jl_quant_robot where status = 1 and platform = "binance"')
    robots = cur.fetchall()
    ...
    if robots:
        for robot in robots:
            ...
            if market_id in market_list:
                ...
                disableRobot(robot_id)
                updateMsg(robot_id, 'market %s not found' % market_id, 'market %s not found' % market_id, 'market %s not found' % market_id)
                insertLog(platform,
            ...
            if "%s-%s" % (platform, uid) in api_list:
                ...
                disableRobot(robot_id)
                updateMsg(robot_id, '%s api 无效 ' % platform, '%s api not found' % platform, '%s api not found' % platform)
                insertLog(platform, robot_id, uid,
