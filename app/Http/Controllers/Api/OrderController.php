...
class OrderController extends Controller
{
    ...
    public function orderInfo(Request $request)
    {
        ...
        $result['today_completed_order'] = BoostOrder::where('user_id', $user->id)->where('round', $round)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->count() + $user->boost_limit;
        $result['user'] = $user;

        if ($user->round == 2) {
            $result['current_round'] = 2;
        } else {
            $result['current_round'] = $user->round;
        }
        ...
        if ($user->day_limit > 0) {
            ...
            if ($user->round == 0) {
                ...
                $limit = $user->package->boost_times2;
            }
        }
    ...
    public function checkOrder(Request $request)
    {
        ...
        $today_completed_order = BoostOrder::where('user_id', $user->id)->where('round', $round)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->count();
        ...
        if ($user->round == 0) {
            ...
            $userSetting = $user->setting2;
        }

        if ($userSetting) {
            $setting = json_decode($userSetting);
    ...
    public function boostOrder(Request $request)
    {
        ...
        $today_completed_order = BoostOrder::where('user_id', $user->id)->where('round', $user->round)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->count();
        $today_completed_order = $today_completed_order + $user->boost_limit;
        $result['user'] = $user;
