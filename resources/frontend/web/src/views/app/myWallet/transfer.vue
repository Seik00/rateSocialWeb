            ...
            <b-col md="5" class="p-0">
              <b-card :title="$t('transfer')">
                <b-form>
                  ...
                  <b-form-group
                    id="input-group-2"
                    :label="$t('receive_username')"
                    label-for="input-2"
                  >
                    <b-form-input
                      id="input-2"
                      v-model="receive_username"
                      type="text"
                      required
                    ></b-form-input>
                  </b-form-group>
  ...
  methods: {
    ...
    getFees() {
      ...
      result
        .then(function (value) {
          self.isLoading = false;
          self.fees = value.data.data.system.TRANSFER_FEE;
        })
        .catch(function (error) {
          self.isLoading = false;
          self.$refs.msg.makeToast("warning", self.$t(handleError(error)));
    ...
    submitForm() {
      let formData = new FormData();
      var self = this;
      formData.append("username", this.receive_username);
