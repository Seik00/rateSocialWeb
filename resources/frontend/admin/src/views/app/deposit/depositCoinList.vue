<template>
  <div class="main-content">
    <breadcumb :page="$t('depositCoinList')" :folder="$t('depositCoin')" />
    ...
    <b-card
      :title="canClear ? $t('searchTotal') + totalRecords : $t('depositCoinList')"
    >
      <vue-good-table
        id="table"
        mode="remote"
      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field == 'document'">
            <span v-if="props.row.uploaded_file.length!=null">
...
<script>
import { getDepositCoinList, controlCoinDeposit } from "../../../system/api/api";
...
export default {
  ...
  methods: {
    ...
    approved(row) {
      ...
      var result = controlCoinDeposit(formData);
      var self = this;
      result
        .then(function (value) {
    ...
    loadItems() {
      var result = getDepositCoinList(this.pageNumber, this.from, this.to, this.searchUsername);
      var self = this;
      this.isLoading = true;
      result
        .then(function (value) {
